...
local eznpc_asset_folder = '/server/assets/ezlibs-assets/eznpcs/'
local custom_events_script_path = 'scripts/events/eznpcs_events'
local custom_events_script_loaded = false
--local generic_npc_mug_animation_path = eznpc_asset_folder..'mug/mug.animation' --I think I made this redundant.
local npcs = {}
local events = require('scripts/ezlibs-scripts/eznpcs/dialogue_types')
local current_player_conversation = {}
local npc_required_properties = {"Direction","Sheet Name"}
local object_cache = {}
...
function eznpcs.get_dialogue_mugshot(npc,player_id,dialogue)
    --local mugshot_asset_name = npc.asset_name --This is the wrong pull.
	local custom_mug_path = dialogue.custom_properties["Mugshot Path"]
    local custom_mug_name = dialogue.custom_properties["Mugshot Name"]
    local texture_path = npc.mug_texture_path
    local animation_path = npc.mug_animation_path
    if custom_mug_name then
		texture_path = eznpc_asset_folder.."mug/"..custom_mug_name..".png"
		animation_path =  eznpc_asset_folder.."mug/"..custom_mug_name..".animation"
		if custom_mug_path then
			if dialogue.custom_properties["ezMugAnimate"] == nil then
				dialogue.custom_properties["ezMugAnimate"] = false
			end
			texture_path = custom_mug_path..custom_mug_name..".png"
			animation_path = custom_mug_path..custom_mug_name..".animation"
		end
		if dialogue.custom_properties["ezMugAnimate"] == nil then
			dialogue.custom_properties["ezMugAnimate"] = true
		end
		if dialogue.custom_properties["ezMugAnimate"] == true then
			animation_path = eznpc_asset_folder.."mug/mug.animation"
		end
    end
    local mug = {}
    mug.texture_path = texture_path
    mug.animation_path = animation_path
    if custom_mug_name == "player" then
...
function create_bot_from_object(area_id,object_id)
    ...
    local npc_sheet_path = eznpc_asset_folder.."sheet/"
    local npc_sheet_name = placeholder_object.custom_properties["Sheet Name"]
    local npc_mug_path = eznpc_asset_folder.."mug/"
	local npc_mug_name = placeholder_object.custom_properties["Sheet Name"]
	--Override Sheet path if it was provided as custom property
    if placeholder_object.custom_properties["Sheet Path"] then
		npc_sheet_path = placeholder_object.custom_properties["Sheet Path"]
	end
	--Override mugshot path if it was provided as custom property
	if placeholder_object.custom_properties["Mugshot Path"] then
		npc_mug_path = placeholder_object.custom_properties["Mugshot Path"]
		if placeholder_object.custom_properties["ezMugAnimate"] == nil then
			placeholder_object.custom_properties["ezMugAnimate"] = false
		end
	end
	--Override mugshot name if it was provided as custom property
    if placeholder_object.custom_properties["Mugshot Name"] then
		npc_mug_name = placeholder_object.custom_properties["Mugshot Name"]
	end
    local npc_texture_path = npc_sheet_path..npc_sheet_name..".png"
    local npc_animation_path = npc_sheet_path..npc_sheet_name..".animation"
    local npc_mug_texture_path = npc_mug_path..npc_mug_name..".png"
    local npc_mug_animation_path = npc_mug_path..npc_mug_name..".animation"
	if placeholder_object.custom_properties["ezMugAnimate"] == nil then
		placeholder_object.custom_properties["ezMugAnimate"] = true
	end
    if placeholder_object.custom_properties["ezMugAnimate"] == true then
		npc_mug_animation_path = eznpc_asset_folder.."mug/mug.animation"
	end
    ...
    local npc = create_npc(placeholder_object.name,npc_texture_path,npc_animation_path,npc_mug_texture_path,npc_mug_animation_path,area_id,x,y,z,direction,npc_turns_to_talk)
...
function create_npc(bot_name,npc_texture_path,npc_animation_path,npc_mug_texture_path,npc_mug_animation_path,area_id,x,y,z,direction,npc_turns_to_talk)
	local name = bot_name or nil
	local texture_path = npc_texture_path
	local animation_path = npc_animation_path
	local mug_texture_path = npc_mug_texture_path
	local mug_animation_path = npc_mug_animation_path
    ...
    local npc_data = {
        ...
        mug_texture_path=mug_texture_path,
...
function eznpcs.create_npc(bot_name,npc_texture_path,npc_animation_path,npc_mug_texture_path,npc_mug_animation_path,area_id,x,y,z,direction,npc_turns_to_talk)
    return ( create_npc(bot_name,npc_texture_path,npc_animation_path,npc_mug_texture_path,npc_mug_animation_path,area_id,x,y,z,direction,npc_turns_to_talk) )
end
